...
- STRICT PROPORTION RULE (most important, do not violate): The full figure must be EXACTLY 3 heads tall from the top of the head to the bottom of the feet — 1 head unit for the head, 2 head units for the combined body (torso + legs). This is a non-negotiable measurement. If you draw a horizontal line across the figure at the chin, and at the halfway point between chin and feet, the body should divide into head / torso / lower-body in roughly 1:1:1 head-units. NOT 2 heads tall (too baby-like, bobblehead). NOT 4+ heads tall (too adult, not chibi). ALWAYS exactly 3 heads tall.
- Style: Colorful JRPG-style 2D chibi sprite art at the 3-heads-tall ratio used by Fire Emblem Heroes, Final Fantasy Tactics, Octopath Traveler character sprites, and MapleStory player characters. The head is sized for that 3-heads ratio — large enough that facial features (eyes, nose, mouth, hair) read clearly at small token sizes, but strictly within the 1/3 vertical budget of the figure. The body takes the remaining 2/3 vertical budget: a real torso, real arms, real legs with visible shoulders, hips, knees, and joints — proportioned for action poses, stylized but clearly articulated. Think of a Fire Emblem Heroes or Octopath Traveler battle sprite: stylized and deformed, reads as a capable adventurer at a glance, precisely 3 heads tall. Thick clean outlines, cel-shaded flat colors, bright saturated JRPG palette, detailed armor/clothing/weapons.`,
...
const STYLE_PROMPT_HINTS: Record<ArtStyle, string> = {
  chibi: "in colorful JRPG chibi sprite art (Fire Emblem Heroes / Final Fantasy Tactics / Octopath Traveler / MapleStory) — STRICTLY 3 heads tall total (1 head + 2 heads of body), no shorter, no taller; head sized so facial features read clearly, body articulated with real torso, arms, and legs occupying the lower 2/3 of the figure",
...
function buildSystemInstruction(artStyle: ArtStyle): string {
  const styleLabel =
    artStyle === "chibi" ? "a JRPG chibi sprite style (Fire Emblem Heroes / Final Fantasy Tactics / Octopath Traveler / MapleStory) with a STRICT 3-heads-tall proportion — the full figure is exactly 3 head-units from top of head to feet (1 head + 2 heads of articulated body). Never shorter (not a 2-heads bobblehead) and never taller (not a 4+ heads adult figure)" :
