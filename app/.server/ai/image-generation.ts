...
- Style: Colorful JRPG-style 2D chibi sprite art. Target head-to-body ratio is approximately 3 heads tall total (including the head) — the same balanced deformed-character ratio used in Fire Emblem Heroes, Final Fantasy Tactics, Octopath Traveler character sprites, and MapleStory player characters. The head is large enough that facial features (eyes, nose, mouth, hair) read clearly at small token sizes, but NOT so oversized that the character looks like a bobblehead, infant, or mascot — avoid the super-deformed 2-heads-tall "baby blob" look AND the near-realistic 4+ heads-tall look. The body is compact but clearly articulated: a real torso, real arms, real legs with visible shoulders, hips, and joints — proportioned for action poses, not shrunk into stubs. Think of how a player character looks in Fire Emblem Heroes or Octopath Traveler battle sprites: stylized and deformed, but still reads as a capable adventurer at a glance. Thick clean outlines, cel-shaded flat colors, bright saturated JRPG palette, detailed armor/clothing/weapons.`,
...
const STYLE_PROMPT_HINTS: Record<ArtStyle, string> = {
  chibi: "in colorful JRPG chibi sprite art (Fire Emblem Heroes / Final Fantasy Tactics / Octopath Traveler / MapleStory) — approximately 3 heads tall, head large enough for clearly readable facial features but NOT a bobblehead, with a compact but fully articulated body (real torso, arms, and legs)",
...
function buildSystemInstruction(artStyle: ArtStyle): string {
  const styleLabel =
    artStyle === "chibi" ? "a JRPG chibi sprite style (Fire Emblem Heroes / Final Fantasy Tactics / Octopath Traveler / MapleStory) — approximately 3 heads tall, head large enough for readable facial features but not a bobblehead, with a compact fully-articulated body" :
